...
##### e.g. 3-1 Discovering co-referenced pages (different source)
Description: Find relationships between pages referenced in content (Page B) and metadata (Page C) of a block (Block A)  
Used First-Degree Methods: B.3, B.2  
...
```clojure
[?block :block/path-refs ?pageB]       ;; Block A references Page B in content
[(missing? $ ?pageB :block/page)]      ;; Confirm Page B is a page
[?pageC :block/name ?pageName]         ;; Get Page C's name
(property ?block :tags ?pageName)      ;; Block A tags Page C in metadata
```

##### e.g. 3-2 Discovering co-referenced pages (same source)
